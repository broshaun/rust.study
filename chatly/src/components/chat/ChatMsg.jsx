...
export const ChatMsg = ({ children, width, height }) => {
...
const Meta = ({ title, left, right }) => (
  <div className={styles.head}>
    ...
    <div className={styles.side} style={{ justifyContent: 'flex-end' }}>{right}</div>
  </div>
);

const Content = ({ children }) => {
  const { contentRef } = useChatMsg();
  return <div ref={contentRef} className={styles.content}>{children}</div>;
};
...
const Send = ({ onSend, placeholder = "输入消息..." }) => {
  ...
  return (
    <div className={styles.send}>
      ...
      />
      <button onClick={handleSend} className={styles.btn}>发送</button>
    </div>
  );
};
