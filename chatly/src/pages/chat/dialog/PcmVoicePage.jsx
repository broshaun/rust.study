      ...
      <div style={{ background: "#fff", padding: 24, borderRadius: 16, boxShadow: "0 4px 20px rgba(0,0,0,0.08)" }}>
        ...
        <div style={{ background: "#f9fafb", padding: 16, borderRadius: 12, marginBottom: 20 }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginBottom: 16 }}>
            <Input label="本地地址" value={v.bindAddr} onChange={e => v.setBindAddr(e.target.value)} />
            <Input label="远端地址" value={v.remoteAddr} onChange={e => v.setRemoteAddr(e.target.value)} />
            <Input label="ServerName" value={v.serverName} onChange={e => v.setServerName(e.target.value)} />
          </div>
        ...
        <div style={{ display: "flex", gap: 12, marginBottom: 20 }}>
          <LargeButton onClick={v.startCapture} disabled={!v.canStartTalk} color="#2563eb">开始讲话</LargeButton>
          <LargeButton onClick={v.stopCapture} disabled={!v.canStopTalk} color="#dc2626">停止</LargeButton>
        </div>
        ...
        {v.lastError && (
          <div style={{ marginTop: 20, padding: 12, background: "#fff1f2", color: "#b91c1c", borderRadius: 8, fontSize: 13 }}>
...
const Input = ({ label, ...props }) => (
  <div>
    ...
    <input {...props} style={{ width: "100%", padding: "8px", borderRadius: 6, border: "1px solid #ddd", boxSizing: "border-box" }} />
  </div>
);

const Button = ({ children, primary, danger, disabled, ...props }) => (
  <button {...props} disabled={disabled} style={{
    padding: "8px 16px", borderRadius: 8, border: "none", fontWeight: "bold", cursor: disabled ? "not-allowed" : "pointer",
...
const LargeButton = ({ children, color, disabled, ...props }) => (
  <button {...props} disabled={disabled} style={{
    flex: 1, padding: "14px", borderRadius: 12, border: "none", color: "#fff", fontWeight: "bold",
    background: color, cursor: disabled ? "not-allowed" : "pointer", opacity: disabled ? 0.4 : 1, fontSize: 16
  }}>{children}</button>
);