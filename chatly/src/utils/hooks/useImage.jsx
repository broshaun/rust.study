...
import { BaseDirectory, mkdir, writeFile, exists, readFile, remove } from "@tauri-apps/plugin-fs";
...
export async function clearAllImageCache() {
  ...
  for (const objectUrl of MEMORY_CACHE.values()) {
    URL.revokeObjectURL(objectUrl);
  }
  MEMORY_CACHE.clear();

  if (await exists(CACHE_DIR, opt)) {
    await remove(CACHE_DIR, { ...opt, recursive: true });
  }
}
