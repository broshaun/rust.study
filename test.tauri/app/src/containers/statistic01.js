...
function StatisticExampleInverted(){
  ...
  useEffect(
    () => {
      // invoke('cmd_a').then((message) => console.log(message))
      setTimeout(
        ()=>{
          invoke('test_rtu').then((rsp) => setParameter(rsp)).catch((e)=>console.log("发生错误：",e))
        },500
      )
    }
  )
  ...
  return <div>
    <Segment inverted>
    <Statistic inverted>
        <Statistic.Value>{parameter}</Statistic.Value>
        <Statistic.Label>RS485</Statistic.Label>
      </Statistic>
    </Segment>
    <Segment inverted>
      <Statistic inverted>
